...
public final class Digests {
	...
	public static <P extends DigestParameters> String buildDigestString(Digest<P> type, P parameters, byte[] salt, byte[] digest) {
		String params = Base64.INSTANCE.getUrlEncoder().encode(type.serialize(parameters));
		String saltString = Base64.INSTANCE.getUrlEncoder().encode(salt);
		String digestString = Base64.INSTANCE.getUrlEncoder().encode(digest);
	...
	public static boolean verify(String digestString, byte[] data) {
		...
		byte[] paramsSerialized = Base64.INSTANCE.getUrlDecoder().decode(match.group(2));
		DigestParameters params = type.deserialize(paramsSerialized);

		byte[] salt = Base64.INSTANCE.getUrlDecoder().decode(match.group(3));
		byte[] actualDigest = Base64.INSTANCE.getUrlDecoder().decode(match.group(4));

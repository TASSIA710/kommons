...
class GreaterEqualAssertion<T : Comparable<T>>(private val value: T, private val limit: T) : Assertion() {
	...
	override fun check() {
		...
		throw AssertionFailure(this) {
			...
			child("What went wrong?") {
				...
				child("A value ($value) was expected to be greater than or equal to a limit ($limit).")
			}
